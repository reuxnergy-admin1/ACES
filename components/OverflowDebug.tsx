...
export default function OverflowDebug() {
  ...
  const [metrics, setMetrics] = useState<{ docW: number; vw: number; culprit?: string } | null>(null);
  useEffect(() => {
    // Allow debug in both dev and prod, but only when explicitly enabled
    const hashOn = typeof window !== 'undefined' && window.location.hash.includes('debug-overflow');
    const pathOn = typeof window !== 'undefined' && window.location.pathname.includes('debug-overflow');
    const qs = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null;
    const qsOn = !!(qs?.has('debug-overflow') || qs?.get('overflowDebug') === '1');
    const lsOn = typeof window !== 'undefined' && window.localStorage.getItem('overflowDebug') === '1';
    ...
    const check = () => {
      ...
      document.querySelectorAll('[data-overflow-culprit]')
        .forEach((el) => { (el as HTMLElement).removeAttribute('data-overflow-culprit'); });
      const doc = document.documentElement;
      const w = Math.max(doc.scrollWidth, doc.clientWidth);
      const vw = window.innerWidth;
      if (w <= vw + 1) { // allow 1px tolerance
      ...
      if (maxEl) {
        ...
        const clsShort = cls ? '.' + cls.toString().split(/\s+/).slice(0, 3).join('.') : '';
  ...
  return (
    <div
      ...
      aria-hidden
    >
      <div><strong>Overflow Debug</strong> (hash, path, query or localStorage)</div>
      {metrics ? (
        <div>
          w: {metrics.docW}px • vw: {metrics.vw}px {metrics.culprit ? `• culprit: ${metrics.culprit}` : ''}
        </div>
      ) : null}
    </div>
  );
