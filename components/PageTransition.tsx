...
export default function PageTransition({
  ...
  // Defer heavy transition effects to reduce TBT during initial load
  ...
  // Defer transition system initialization until after page is interactive
  useEffect(() => {
  ...
  useEffect(() => {
    const isAuto =
      ...
      Reflect.get(navigator, "webdriver") === true;
  ...
  const finalizeAfterShrink = useCallback(
    (overlayEl: SVGSVGElement, dimNode: SVGRectElement | null) => {
  ...
  const fallbackRevealIfActive = useCallback(() => {
    ...
    shrinkCircle(circ, dimNode, timings.current.revealDur).then(() =>
      finalizeAfterShrink(ov, dimNode),
    );
  }, [finalizeAfterShrink]);
    ...
    if (ov.dataset.active === "1") {
      ...
      if (circ) {
        shrinkCircle(circ, dimNode, timings.current.revealDur).then(() => {
          ...
          if (overlayEl) finalizeAfterShrink(overlayEl, dimNode);
        });
      } else {
        clearLocksAndOverlay();
      }
  ...
  useEffect(() => {
    if (!ready) return; // defer until after initial paint
    ...
    function preLock(e: PointerEvent) {
      ...
      if (ov && c && ov.dataset.active !== "1") {
        ...
        window.setTimeout(
          fallbackRevealIfActive,
          timings.current.fallbackRevealAfter,
        );
        armWatchdog();
      }
    }
    function onClick(e: MouseEvent) {
      ...
      function frame(t: number) {
        ...
        const dim = dimRef.current;
        if (dim) dim.setAttribute("opacity", String(Math.min(0.2, p * 0.2)));
        if (p < 1) requestAnimationFrame(frame);
      }
      requestAnimationFrame(frame);
    ...
    return () => {
      ...
      document.removeEventListener("pointerdown", preLock, true);
    };
  }, [
    ready,
    armWatchdog,
  ...
  useEffect(() => {
    ...
    if (!ov) return;
    function setVB() {
      (ov as SVGSVGElement).setAttribute(
        "viewBox",
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      if (!overlayRef.current || overlayRef.current.dataset.active === "1")
        return;
    ...
    return () =>
      window.removeEventListener("popstate", handler as EventListener, opts);
  }, [ready, armWatchdog, clearLocksAndOverlay, isReducedMotionNow]);
    ...
    <>
      ...
      >
        <defs>
          <filter id={filterId}>
            <feTurbulence
              ref={(el: SVGFETurbulenceElement | null) => {
                noiseRef.current = el;
              }}
              type="fractalNoise"
              numOctaves="2"
