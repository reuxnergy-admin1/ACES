...
export default async function RootLayout({ children }: Readonly<{ children: ReactNode }>) {
  ...
  <script nonce={nonce}>{`(()=>{try{var reduce=(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)===true;if(reduce)return;var isAuto=!!(navigator&&navigator.webdriver===true);function isInternal(a){if(!a)return false;var h=a.getAttribute?(a.getAttribute('href')||''):'';return h.indexOf('/')===0&&h.indexOf('/#')!==0}function clear(){document.body.classList.remove('cursor-hide-transition','pt-disable-scroll','pt-no-events');var ov=document.querySelector('svg.pt-blob-svg');if(ov)ov.setAttribute('data-active','0')}function markOverlayActive(){var ov=document.querySelector('svg.pt-blob-svg');if(ov){ov.setAttribute('data-active','1');ov.style.pointerEvents='none';return true}return false}if(isAuto){setTimeout(function(){try{document.body.classList.add('cursor-hide-transition','pt-disable-scroll','pt-no-events')}catch(_){};if(!markOverlayActive()){var mo=new MutationObserver(function(){if(markOverlayActive()){try{mo.disconnect()}catch(_){}}});try{mo.observe(document.documentElement,{childList:true,subtree:true})}catch(_){}}setTimeout(clear,2200)},50)}function onClick(e){var t=e.target;if(!t||!t.closest)return;var a=t.closest('a[href]');if(!isInternal(a))return;document.body.classList.add('cursor-hide-transition','pt-disable-scroll','pt-no-events');if(isAuto){setTimeout(clear,2200)}}document.addEventListener('click',onClick,true)}catch(_){}})();`}</script>
      </head>
